...
export class RestMultimedija {
  ...
  private uploadDir = "podaci/multimedija";
  
  // Dozvoljeni tipovi datoteka
  private dozvoljeneTipoveSlike = ["image/jpeg", "image/png", "image/gif", "image/webp"];
  private dozvoljeneTipoviVideo = ["video/mp4", "video/webm", "video/quicktime"];
  
  // Maksimalne veličine (u bajtima)
  private maksVelicinaSlike = 500 * 1024; // 500 KB
  private maksVelizinaVidea = 1 * 1024 * 1024; // 1 MB
   ...
   */
  private detektujTipMultimedije(mimeType: string): string | null {
   ...
   */
  private validacijaVeličineUKorisnika(tip: string, veličina: number): boolean {
    if (tip === "slika") {
   ...
   */
  private očistiNazivDatoteke(naziv: string): string {
    return naziv
   ...
   * Upload nove multimedije sa validacijom
   */
  async postMultimedija(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      // Obriši datoteku ako je nešto pošlo po zlu
      if (req.file?.path) {
        await fs.unlink(req.file.path).catch(() => {});
      }
  ...
  async deleteMultimedija(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      // Provjeri dozvole pristupa
      // Admin i moderator mogu brisati sve
      // Ostali korisnici mogu brisati samo ako su vlasnici kolekcije ili ako je kolekcija javna
   ...
   * Dohvati metadata multimedije sa proverom dozvola
   */
  ...
  async getMultimedija(req: Request, res: Response) {
    ...
    const uloga = req.session?.korisnik?.uloga;
    const korisnikId = req.session?.korisnik?.id;
    const kolekcijaIdParam = req.query['kolekcijaId'] as string | undefined;
  ...
  /**
   * PUT /api/multimedija/:id
   * Ažuriraj metadata multimedije
   */
